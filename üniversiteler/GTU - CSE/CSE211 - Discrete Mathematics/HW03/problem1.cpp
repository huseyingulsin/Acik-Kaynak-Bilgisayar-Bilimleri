...
public:
    ...
    // ---- HW3 Implementations ----
    // I will write, which lines is different from HW2
    // For you to checking quickly hocam.

    // writeValues() function changed. start to end.
    //  Reflexive -> Line 170, Antysmetric -> Line 265, Transitivity ->  Line 330
};
...
void Relation::writeValues(ofstream& fp)
{
    ...
    // --- HW3 ---
    // In those functions
    // pairs will be updated due to, which pairs is missing
    // when all missing values is added, stop while loops
    while(!reflexive());
    while(!antisymmetric());
    while(!transitive());

    fp << "POSET: ";
    for (int i = 0; i < pairs.size(); i++)   // print all pairs of element in file
        fp << "(" << pairs[i] << ") ";
    fp << endl;

    vector <string> newPairs;       // create new string vector
    for (int i = 0; i < pairs.size(); i++)
    {
        if (pairs[i][0] != pairs[i][2])
            newPairs.push_back(pairs[i]);   // add values which are not reflexive
    }

    // determine and erase transitive values.
    for (int i = 0; i < newPairs.size(); i++)
    {
        for (int j = 0; j < newPairs.size(); j++)
        {
            if (newPairs[i][2] == newPairs[j][0])
            {
                for (int k = 0; k < newPairs.size(); k++)
                {
                    if (newPairs[k][0] == newPairs[i][0] && newPairs[k][2] == newPairs[j][2])
                        newPairs.erase(newPairs.begin() + k);
                }
            }
        }
    } 

    if (newPairs.size() == 0)       // if all values reflexive, it will be empty.
        fp << "This set is empty (probably, all values are reflexive)" << endl;
    else
        for (auto t = newPairs.begin(); t != newPairs.end(); ++t)
            fp << *t << endl;
...
bool Relation::reflexive()
{
    ...
    else
    {
        ...
        // --- HW3 ---
        pairs.push_back(tmp);   // add missing reflesive part
        pairNum++;              // increment pair size
...
bool Relation::antisymmetric()
{
    ...
    {
        // --- HW3 ----
        // Erase a the symetric value
        for (auto iter = pairs.begin(); iter != pairs.end(); ++iter)
        {
            if (reversePair == *iter)
                pairs.erase(iter);
        }
        pairNum--; // discrement pair size
...
bool Relation::transitive()
{
    ...
    else
    {
        ...
        // ---- HW3 -----
        pairs.push_back(tmp); // add missing part for transitivity
        pairNum++;            // increment pair size
